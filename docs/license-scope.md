...
AGPL-3.0-only`). [LICENSE](../LICENSE) contains the unmodified license text published by GNU.
...
- any other external asset identified in [THIRD_PARTY.md](../THIRD_PARTY.md), source registries, or audit records.

No third-party image, icon set, font, engine binary, evaluation file, bulk dataset, checkpoint,
or trained model weight is included. Small attributed regression fixtures remain under `tests/fixtures/`;
their provenance and source-specific terms are not replaced by the source-code license.
