...
impl EvaluatorChoice {
    fn parse(value: &str) -> Result<Self, String> {
        match value {
            ...
            _ => Err(
                "evaluator must be overall-champion, model, model-residual, model-composite, or pure_learned"
                    .to_owned(),
            ),
        }
    }
...
impl BrowserEngine {
    ...
    pub fn load_model(
        ...
        self.model = Some(LoadedModel {
            evaluator,
            summary,
            expected_hash_verified: expected_artifact_sha256.is_some(),
        });
        self.analysis = None;
    ...
    fn search_engine(
        ...
        match evaluator {
            ...
            | EvaluatorChoice::PureLearned => {
                ...
                if evaluator == EvaluatorChoice::PureLearned && !model.expected_hash_verified {
                    return Err(
                        "pure_learned requires an explicitly verified model artifact SHA-256"
                            .to_owned(),
                    );
                }
                match &model.evaluator {
                    LoadedEvaluator::Osaval01(_) if evaluator == EvaluatorChoice::PureLearned => {
                        Err("pure_learned requires an OSAVAL02 model".to_owned())
                    }
                    ...
                    LoadedEvaluator::Osaval02(osaval02) => {
                        ...
                        if evaluator == EvaluatorChoice::PureLearned {
                            SearchEngine::with_pure_learned(
                                config,
                                Arc::clone(osaval02),
                                &osaval02.identity().artifact_sha256,
                            )
                        } else {
    ...
    fn runtime_proof(
        ...
        let model_sha256 = match &self.model.as_ref()?.evaluator {
            LoadedEvaluator::Osaval02(model) => model.identity().artifact_sha256.clone(),
            LoadedEvaluator::Osaval01(_) => return None,
        };
        Some(engine.runtime_proof(stats, model_sha256))
    }
...
mod tests {
    ...
    fn browser_search_uses_the_fixed_profile_budget() {
        ...
        assert!(engine.search_json("eco", "pure_learned", 1).is_err());
