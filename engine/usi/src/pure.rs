...
fn score_field(score: i32) -> String {
    ...
    let distance = open_shogi_core::MATE_SCORE
        .saturating_sub(score.saturating_abs())
        .max(0);
...
mod tests {
    ...
    use open_shogi_core::{MATE_SCORE, Position};
    #[test]
    fn scores_preserve_exact_mate_namespace() {
        assert_eq!(score_field(123), "cp 123");
        assert_eq!(score_field(MATE_SCORE - 3), "mate 2");
        assert_eq!(score_field(-MATE_SCORE + 3), "mate -2");
    }
    #[test]
